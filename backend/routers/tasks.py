...
def get_task_history(
    ...
    query = (
        select(TaskEventLog)
        .where(
            TaskEventLog.user_id == user_id, TaskEventLog.timestamp >= seven_days_ago
        )
...
# -----------------------------CREATE TASKS------------------------------------
@router.post(
    "/create", status_code=fastapi_status.HTTP_201_CREATED, response_model=TaskModel
)
def create_tasks(
...
def update_task(
    ...
    query = select(Task).join(User).where(Task.id == id, User.email == token_data.email)
    ...
    for key, value in update_task_data.items():
        if key == "status":
            old_status = (
                db_task.status.name
                if hasattr(db_task.status, "name")
                else db_task.status
            )
            ...
            if old_status != new_status:
                ...
                if new_status == "COMPLETED" and started:
                    ...
                    if completed_time is not None:
                        if (
                            hasattr(completed_time, "tzinfo")
                            and completed_time.tzinfo is None
                        ):
                        ...
                        if duration_sec > 0:
                            ...
                            dur_str = (
                                f"{h}h {m}m {s}s"
                                if h > 0
                                else (f"{m}m {s}s" if m > 0 else f"{s}s")
                            )
            ...
            old_pri = (
                db_task.priority.name
                if db_task.priority and hasattr(db_task.priority, "name")
                else str(db_task.priority)
            )
...
def get_task(
    ...
    if not db_task:
        raise HTTPException(
            status_code=fastapi_status.HTTP_404_NOT_FOUND, detail="TASK not found"
        )
...
def delete_task(
    ...
    log = TaskEventLog(
        task_id=task_id, task_title=task_title, user_id=user_id, event_type="DELETE"
    )
    db.add(log)
