...
def analytics( db: Session = Depends(get_db),token_data : TokenData = Depends(verify_token)):
    ...
    analytical_query = (
        select(
            ...
            func.sum(case((Task.status == Status.COMPLETED, 1), else_=0)).label("COMPLETED"),
            func.sum(case((Task.status == Status.IN_PROGRESS, 1), else_=0)).label("IN_PROGRESS")
        )
