...
def user(token_data: TokenData = Depends(verify_token), db: Session = Depends(get_db)):
    is_blocked = db.scalar(select(TokenBlocklist).where(TokenBlocklist.jti == token_data.jti))
    if is_blocked:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def refresh_session(payload: RefreshTokenRequest, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail="Expired or broken token"
        )
...
def logout(
    ...
    try:
        decoded_refresh = jwt.decode(payload.refresh_token, SECRET_KEY, algorithms=[ALGORITHM])
        ...
        if not refresh_jti or not refresh_exp or not isinstance(refresh_exp, (int, float)):
            raise HTTPException(status_code=400, detail="Invalid refresh token metadata structure")
            
        if email_sub != token_data.email or token_type != "refresh":
            raise HTTPException(status_code=400, detail="Invalid refresh token payload mapping match")
            
    except JWTError:
        raise HTTPException(status_code=400, detail="Broken or expired refresh token configuration")
